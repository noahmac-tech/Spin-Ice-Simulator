...
int main() {
    ...
    // B-Field is ZERO here to allow the degenerate ice-rule manifold to form
    ...
    // Setup output file (Ensure you have a 'data' folder created)
    fstream all_data_file;
    all_data_file.open("data/data_low_temp_entropy.txt", ios::out);
    all_data_file << "T0\tAverage_Energy\tAverage_Magnetisation\tSpecific_Heat\tSusceptibility\n";

    // Run Simulation over Temperatures (starting from T = 12.0)
    ...
    all_data_file.close();
    cout << "Low temperature entropy data collection finished!\n";
    return 0;
}