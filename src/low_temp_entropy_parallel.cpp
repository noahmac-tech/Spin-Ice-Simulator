...
SimulationResult run_simulation_for_temperature(double T0, int L, double a, double J_eff, double Bx, double By, double Bz, int eq_sweeps, int meas_sweeps, int sweeps_between) {
    ...
    // Unique seed for each thread based on time and temperature
    random_device rd;
...
int main() {
    ...
    // B-fields MUST be zero to allow residual entropy to form
    ...
    cout << "Launching multithreaded Entropy simulations across all CPU cores...\n";

    // Dispatch a thread for every single temperature (from 12.0 down to 0.1)
    for (double T0 = 12.0; T0 >= 0.1; T0 -= 0.1) {
        futures.push_back(
    ...
    // Write sorted results to parallel text file
    fstream all_data_file;
    all_data_file.open("data/data_low_temp_entropy_parallel.txt", ios::out);
    ...
    all_data_file.close();
    cout << "All threads finished successfully! Data saved to data/data_low_temp_entropy_parallel.txt\n";
    return 0;
}